...
def run( seed, noise_type, layer_norm, nb_epochs, nb_epoch_cycles, reward_scale,
    ...
    env = TorcsEnv(vision=vision, throttle=True, gear_change=False,
		...
		lap_limiter=lap_limiter, randomisation=True, profile_reuse_ep=20,
        port=4001)
    ...
    with U.single_threaded_session() as sess:
        ...
        traj_data = { "obs": [], "acs": [], "rews": [], "ep_rets": []}
        traj_limitation = 200
        traj_length = 3600
        ...
        while True:
            ...
            while sampled_count < traj_length:
                ...
                print( "Sampled cunt so far: %d" % sampled_count)
                # print( "Sampled cunt so far 2: %d" % len( obss))

            obss = obss[0:traj_length]
            acss = acss[0:traj_length]
            rews = rews[0:traj_length]

            traj_data["obs"].append( obss)
            traj_data["acs"].append( acss)
            traj_data["rews"].append( rews)

            traj_data["ep_rets"].append( episode_reward)

            # Truncate to max size of traj_data
            if( len( traj_data["obs"]) > traj_limitation):
                traj_data["obs"] = traj_data["obs"][-traj_limitation:]
                traj_data["acs"] = traj_data["acs"][-traj_limitation:]
                traj_data["rews"] = traj_data["rews"][-traj_limitation:]
                traj_data["ep_rets"] = traj_data["ep_rets"][-traj_limitation:]

            # print( "### DEBUG: Sampled traj so far: %d" % len( traj_data["obs"]))
            rcon.set( "rl_traj_data", dumps( traj_data))
